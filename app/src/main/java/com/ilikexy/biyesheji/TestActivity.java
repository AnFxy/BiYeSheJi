...
public class TestActivity extends BaseActivity implements View.OnClickListener{
    ...
    private TextView tvOrder,tvType;
...
//                    "短期激励与长期激励相协调","C");
    ...
    public void checkIsRightTi(){//核查是否做对了题目
        ...
        for (int i=0;i<listOfFragment.size();i++){
            ...
            if (!yourChoose.equals(tiAnswerr.getmAnswer())){//做错题了，上传到服务器
                sendTheWrongRecord(getUsename(),listOfFragment.get(i).tiAnswer.getTiUId(),yourChoose);
            }else{
                theRightcount++;
            }
        }//展示做题结果,跳转到新的活动,生成做题记录，用户ID，做题总数，做对题数,做题时间，做题耗时，存储至服务器
        sendDoTiRecord(getUsename(),listOfFragment.size(),theRightcount,ArticlerActivity.getTimeNow());

    }//做错题了，上传到服务器
    public void sendTheWrongRecord(final String usename, final String tiuid, final String wrongchoose){
        new Thread(new Runnable() {
            @Override
            public void run() {
                OkHttpClient client = new OkHttpClient();
                Request request = new Request.Builder()
                        .url(ConstantClass.STRING_SERVICE_URL+ConstantClass.STRING_SERVICE_PROJECTNAME+
                                "AddWrongTiRecord?"+"usename="+usename+"&tiuid="+tiuid+"&wrongchoose="+wrongchoose)
                        .build();
                Call call = client.newCall(request);
                call.enqueue(new Callback() {
                    @Override
    ...
    public void sendDoTiRecord(final String usename, final int counts, final int right, final String time){
        new Thread(new Runnable() {
            @Override
            public void run() {
                OkHttpClient client = new OkHttpClient();
                Request request = new Request.Builder()
                        .url(ConstantClass.STRING_SERVICE_URL+ConstantClass.STRING_SERVICE_PROJECTNAME+
                                "SendDoTiRecord?"+"usename="+usename+"&counts="+counts+"&right="+right+"&item="+time)
                        .build();
                Call call = client.newCall(request);
                call.enqueue(new Callback() {
                    @Override
